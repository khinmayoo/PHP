...
if (isset($_POST['register'])) {
    ...
    if (empty($user_name)) {
        ...
        try {
            ...
            $sql = "SELECT name, email FROM sample_php.users WHERE name =  ? and email = ? ";

            // Prepare the statement
            $query = $conn->prepare($sql);
            $query->bind_param('ss', $user_name, $user_email);
            ...
            if ($row && isset($row['name']) && $row['name'] == $user_name) {
                array_push($errors, "That username is taken. Please choose something different.");
            } elseif ($row && isset($row['email']) && $row['email'] == $user_email) {
